...
function addInstance(instance) {
  var elementName = instance.elementName;
  ...
  createClass(StyleInfo, null, [{
    ...
    value: function invalidate(elementName) {
      if (templateMap[elementName]) {
        ...
        if (nativeShadow) {
          var instances = instanceMap[elementName];
          if (!instances) {
            return;
          }
    ...
    key: 'validate',
    value: function validate(elementName) {
      templateMap[elementName]._applyShimInvalid = false;
    }
  }]);

  function StyleInfo(ast, placeholder, ownStylePropertyNames, elementName, typeExtension, cssBuild, applyShimInvalid) {
...
if (!nativeShadow) {
  (function () {
    var elementNeedsScoping = function elementNeedsScoping(element) {
      ...
      element instanceof SVGElement && element.getAttribute('class').indexOf(StyleTransformer.SCOPE_NAME) < 0;
    };
...
var ShadyCSS = {
  ...
  _prepareHost: function _prepareHost(host) {
    ...
    return StyleInfo.set(host, new StyleInfo(ast, placeholder, ownStylePropertyNames, is, typeExtension, cssBuild, applyShimInvalid));
  },
  ...
  _isElementNode: function _isElementNode(node) {
    return node.nodeType === Node.ELEMENT_NODE;
  },
  _applyToDescendants: function _applyToDescendants(children) {
