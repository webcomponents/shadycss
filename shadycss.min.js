...
var StyleProperties = {
  ...
  collectProperties: function collectProperties(rule, properties) {
    ...
    if (info) {
      if (info.properties) {
        use(properties, info.properties);
        return true;
      }
    } else {
      var m = void 0,
  ...
  createClass(ApplyShim, [{
    ...
    value: function _produceCssProperties(matchText, propertyName, valueProperty, valueMixin) {
      ...
      if (oldProps) {
        ...
        combinedProps = use(Object.create(oldProps), mixinValues);
      } else {
...
var ShadyCSS = {
  ...
  applyStyle: function applyStyle(host, overrideProps) {
    ...
    if (overrideProps) {
      ...
      use(styleInfo.overrideStyleProperties, overrideProps);
    ...
    if (this._forceRecalc || styleInfo.shouldRecalculate || template._applyShimInvalid) {
      if (this.nativeCss) {
        if (template && template._style && template._applyShimInvalid) {
          ...
          if (this.nativeShadow) {
            var _root = host.shadowRoot;
            if (_root) {
              var style = _root.querySelector('style');
  ...
  applyStyleInvalid: function applyStyleInvalid(node) {
    var styleInfo = StyleInfo.get(node);
    if (styleInfo) {
  ...
  _updateProperties: function _updateProperties(host, styleInfo) {
    ...
    use(props, hostAndRootProps.hostProps, propertiesMatchingHost, hostAndRootProps.rootProps);
